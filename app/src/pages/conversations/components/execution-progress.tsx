...
export const ExecutionProgress: FC<ExecutionProgressProps> = ({ toolCalls, isRunning, className }) => {
  ...
  return (
    <div className={cn("mr-auto flex w-fit max-w-[min(92%,22rem)] flex-col gap-2 sm:max-w-[min(85%,22rem)]", className)}>
      {isRunning && (
        <div className="w-fit rounded-2xl bg-surface-secondary px-3 py-2.5">
          <AiTypingIndicator />
        </div>
      )}
      ...
      {hasTools && (
        <div className="w-full overflow-hidden rounded-xl border border-border/40 bg-muted/25">
          <button type="button" onClick={() => setExpanded((value) => !value)} className="flex w-full items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-muted/35" aria-expanded={expanded}>
            <ChevronRight className={cn("size-3.5 shrink-0 text-muted transition-transform duration-200", expanded && "rotate-90")} aria-hidden />
            <span className="min-w-0 flex-1 truncate text-xs text-muted">{getActivityHeaderLabel(toolCalls, isRunning)}</span>
            {isRunning && toolCalls.every((tool) => tool.status !== "running") && <Loader2 className="size-3 shrink-0 animate-spin text-muted" aria-hidden />}
          </button>

          {expanded && (
            <div className="space-y-0.5 border-t border-border/30 px-3 py-2" role="log" aria-live="polite" aria-label="Tools used by the assistant">
              {toolCalls.map((tool) => (
                <div key={tool.callId} className={cn("flex items-center gap-2 py-1 text-xs", tool.status === "running" ? "text-foreground" : "text-muted")}>
