...
import { SENDGRID_REQUIRED_CONFIG_KEYS } from './config/sendgrid.config';
import { SendGridService } from './services/sendgrid.service';
import { verifySendGridApiKey } from './utils/sendgrid.utils';

@Injectable()
export class SendGridIntegration extends SaasIntegration {
  ...
  protected readonly actions: SaasActionDefinition[] = [
    {
      key: 'send_email',
      label: 'Send email',
      description: 'Send a plain-text/HTML email via SendGrid.',
    ...
    },
    {
      ...
      description: 'Send an email with separate plain-text and HTML parts via SendGrid.',
    ...
    },
    {
      ...
      description: 'Send an email with one or more base64-encoded file attachments via SendGrid.',
    ...
    },
    {
      ...
      description: 'Send the same email to a list of recipients via SendGrid.',
    ...
    },
    {
      ...
      description: 'Verify that the SendGrid API key is valid.',
      schema: emptySchema,
  ...
  protected requiredConfigKeys() {
    return [...SENDGRID_REQUIRED_CONFIG_KEYS];
  }
  ...
  async testConnection(config: Record<string, any>): Promise<boolean> {
    ...
    return verifySendGridApiKey(String(config.apiKey));
  }

  protected async executeValidatedTool(actionKey: string, input: Record<string, any>, config: Record<string, any>) {
    const service = new SendGridService(String(config.apiKey), String(config.from));
