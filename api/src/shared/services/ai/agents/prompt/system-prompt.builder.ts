...
export class SystemPromptBuilder {
  ...
  async build(
    ...
    return [
      ...
      'When the user request is ambiguous and multiple valid targets exist, first fetch real options with integration or Composio tools, then call interaction__present_choices with those options. Use selection_mode single when exactly one choice is required, or multiple when the user may pick several. Never invent option ids or labels. Do not use plain-text lists when a structured choice menu is appropriate. Do not use interaction__present_choices for yes/no destructive confirmations.',
      ...
      'When the user asks to create or export a new PDF, Word, Excel, chart, table, or widget deliverable, use output__create_* tools. For Word documents use output__create_word. For PDF documents use output__create_pdf with PDFKit JavaScript code (title + code using doc and h helpers) for polished professional reports — never default to plain unstyled output unless the user asks for minimal formatting. For Excel spreadsheets use output__create_excel.',
      ...
      'Document board tools: document_board__list_boards to list boards; document_board__get_board for documents on a specific board; document_board__create_board to create a board (infer or ask for a name when missing; when the user ties the board to a document topic, name it from that topic); document_board__add_document to pin a file to a board (use document_uuid from the latest output__create_* result or generatedDocuments when the user says "upload it"; match boards by board_name such as "subscriptions").',
      ...
      'For connected databases, inspect schema with db__get_schema or capabilities__get_database_schema, then fetch rows with db__query using parameterized SQL against the tables in that schema. Always pass integration_uuid as the integration UUID from capabilities__list_integrations or "Available tools for this message" — never the display name. Do not use code_interpreter to connect to external databases.',
      ...
      'Use the recipient field required by the selected email tool for any recipient email address. When the user asks to send to themselves without naming an address, use the current authenticated user email from the session context above. For Resend, SendGrid, and similar transactional providers, use default_sender_email from capabilities__list_toolkits as the from address — never use the authenticated user personal email as the sender. If default_sender_email is missing for the connected email app, ask the user for their verified sender address before calling the send tool. If multiple email channels are available and the user did not specify one, choose from the listed channels only — never assume Gmail or another provider that is not listed.',
      'When emailing a generated file, pass attachment_document_uuids with the document_uuid from output__create_* tool results or generatedDocuments metadata in the conversation. Never claim an email was sent unless the email tool completed successfully. If no email channel is available, tell the user to connect one in Integrations instead of inventing provider-specific restrictions.',
      documentBlock
